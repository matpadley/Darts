...
namespace DartsScorer.Main.Functions;

public class DartThrowConverter
{
    public (BoardScore boardScore, Multiplier boardMultiplier) Convert(string dartThrow)
    {
        if (int.TryParse(dartThrow, out _)) dartThrow = "S" + dartThrow;

        if (dartThrow == "25" || dartThrow == "50" || dartThrow == "Bull")
        {
            return (dartThrow == "25" ? BoardScore.OuterBull : BoardScore.BullsEye, Multiplier.Single);
        }

        var regexString = "^(S|D|T)(1[0-9]|20|[1-9])$|^(25|BULL|50)$";
        ...
        return (boardScore, boardMultiplier);
    }
}